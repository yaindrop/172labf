...
//*****************************************************************************
//
// Application Name     -   MQTT Client Based Screen Driver
...
#define SERVER_ADDRESS           "192.168.86.28"
//#define SERVER_IP_ADDRESS        "192.168.86.28"
...
#define TOPIC4                  "/cc3200/ScreenControl"
...
#define CONTINUE_IF_TRUE(condition) {if(condition) continue;}
                       ...
                       long pay_len, bool dup,unsigned char qos, bool retain)
{
    ...
    UART_PRINT("\n\r");
    if (isScreenControl) {
        int size, cursorX, cursorY;
        char text[24];
        sscanf(output_str, "%d %d %d %[^\t\n]", &size, &cursorX, &cursorY, text);
        if (size) {
            setTextSize(size);
...
void testbands(unsigned char vertical) {
    unsigned int ulColors[] = {BLACK, BLUE, GREEN, CYAN, RED, MAGENTA, YELLOW, WHITE};
    int i;
    for (i = 0; i < 8; i ++) {
        if (vertical) {
            fillRect(i << 4, 0, 16, SSD1351HEIGHT, ulColors[i]);
        } else {
            fillRect(0, i << 4, SSD1351WIDTH, 16, ulColors[i]);
        }
    }
}
